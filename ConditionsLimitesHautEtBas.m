...
%         A(k1,k1+1)=A(k1,k1+1)-hc+lambdaair/(2*dx)-lambda*dt/(rho*c_p*dx.^2);
        ...
        A(index(i,j),index(i,j))=A(index(i,j),index(i,j))+1+3*hcairdalle*dt/(2*rho*c_p*dx^2)-3*lambda*dt/(rho*c_p*dx^2);
        A(index(i,j),index(i,j-1))=A(index(i,j),index(i,j-1))+2*lambda*dt/(rho*c_p*dx^2)-hcairdalle*dt/(rho*c_p*dx^2);
        A(index(i,j),index(i,j-2))=A(index(i,j),index(i,j-2))-lambda*dt/(rho*c_p*dx^2);
        A(index(i,j),index(i,j+1))=A(index(i,j),index(i,j+1))-hcairdalle*dt/(rho*c_p*dx^2);
...
%          B(k2)=Tavant(k2);
        ...
        A(index(i,j),index(i,j-1))=A(index(i,j),index(i,j-1))-3*hcairdalle*dt/(2*rhoair*c_p_air*dx^2);
        A(index(i,j),index(i,j+1))=A(index(i,j),index(i,j+1))-hcairmurs*dt/(rhoair*c_p_air*dx^2);
        A(index(i,j),index(i,j-2))=A(index(i,j),index(i,j-2))+hcairdalle*dt/(rhoair*c_p_air*dx^2);
        A(index(i,j),index(i,j))=A(index(i,j),index(i,j))+1+hcairdalle*dt/(rhoair*c_p_air*dx^2)+hcairmurs*dt/(rhoair*c_p_air*dx^2)-2*lambdaair*dt/(rhoair*c_p_air*dx^2);
        %if i>2 && i<(noeudsHor-1)
...
%             A(index(i,j),index(2,j))=A(index(i,j),index(2,j))-lambdaair*dt/(rhoair*c_p_air*dx^2);
        ...
        A(index(i,j),index(i,j))=A(index(i,j),index(i,j))+1-2*lambdamurs*dt/(rhomurs*c_p_murs*dx^2)-hcairmurs*dt/(rhomurs*c_p_murs*dx^2)-hcairmurs*dt/(rhomurs*c_p_murs*dx^2);
        A(index(i,j),index(i,j-1))=A(index(i,j),index(i,j-1))+hcairmurs*dt/(rhomurs*c_p_murs*dx^2);
