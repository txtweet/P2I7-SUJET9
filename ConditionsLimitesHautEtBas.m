...
%     A(index(i,j),index(i,j-1))=A(index(i,j),index(i,j-1))+2*lambda*dt/(rho*c_p*dx^2)-hcairdalle*dt/(2*rho*c_p);
...
%     A(index(i,j),index(i,j-2))=A(index(i,j),index(i,j-2))+hcairdalle*dt/(2*rhoair*c_p_air);
...
%     A(index(i,j),index(i,j-1))=A(index(i,j),index(i,j-1))-2*dt/(rho*c_p*((dx*dx/lambda)+(eisolant*dx/lambdaisolant)));
...
% %% CONDITIONS PERIODIQUES HAUT PLANCHER
% j=noeudsVert-2;
% 
% i=1;
% %gauche
...
% A(index(i,j),index(1,j))=A(index(i,j),index(1,j))+ksolidedalle;
% 
% %% CONDITIONS PERIODIQUES NOEUD D'AIR
% j=noeudsVert-1;
% 
% i=1;
% %gauche
...
% A(index(i,j),index(1,j))=A(index(i,j),index(1,j))+kcondair;
% 
% %% CONDITIONS PERIODIQUES NOEUD DE MURS
% j=noeudsVert;
% 
% i=1;
% %gauche
...
% A(index(i,j),index(1,j))=A(index(i,j),index(1,j))+kcondmur;
% 
% %% CONDITIONS PERIODIQUES AU BAS DE LA BALLE
% j=2;
% 
% i=1;
% %gauche
...
% A(index(i,j),index(i+1,j))=A(index(i,j),index(i+1,j))-4*ksolidedalle;
% A(index(i,j),index(1,j))=A(index(i,j),index(1,j))+ksolidedalle;
% %% CONDITIONS PERIODIQUES AU NOEUD D'ISOLANT
% j=1;
% 
% i=1;
% %gauche
...
% Autre méthode qui évite les boucles if dans la grande boucle for (plus rapide lors de l'exécution).
%     %% Au niveau du haut du plancher (j=N-2)
%     j=noeudsVert-2;
%     i=2;
%     A(index(i,j),index(i+1,j))=A(index(i,j),index(i+1,j))+2*lambda*dt/(rho*c_p*dx^2);
