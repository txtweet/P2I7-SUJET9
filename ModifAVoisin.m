...
global dx l rho c_p noeudsVert hc dt Tchauf B lambda
    ...
    ksolide=-lambda*dx*dt*l/(rho*c_p*2*dx);     %Coefficient multiplicateur pour la derivée du solide
    kfluide=dx*dt*l*hc/(rho*c_p);               %Coefficient multiplicateur pour la derivée dans le fluide
    ...
    if (Voisin(1,2)==0)     %Voisin du haut
        A(coord,coord)= A(coord,coord)+3;
        A(coord,coord-1) = -4;
        A(coord,coord-2) = 1;
    end 
    if (Voisin(1,3)==0)     %Voisin du bas 
        A(coord,coord)= A(coord,coord)-3;
        A(coord,coord+1) = 4;
        A(coord,coord+2) = -1;
    end
    if (Voisin(1,4)==0)     %Voisin de droite
        A(coord,coord)= A(coord,coord)-3;
        A(coord,coord+noeudsVert) = 4;
        A(coord,coord+2*noeudsVert) = -1;
    end
